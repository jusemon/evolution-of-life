  ...
  const loop = GameLoop({
    update: function (dt) {
      ...
      // Idle
        ...
        player.dx =
          Math.round(player.dx * deceleration * floatingLimit) / floatingLimit;
      ...
      if (player.jumping()) {
        ...
        if (player.jumpingUp) {
          if (player.y > player.jumpingHeightPos) {
            player.y = Easing.easeOut(
              time,
              player.jumpingStartPos,
              player.jumpingHeightPos,
              0.4
            );
          } else {
            time = 0;
      ...
      const offset = { x: 5, w: -10 };
      const isCollidingWithGround = tileEngine.layerCollidesWith(
        Layers.Ground,
        collitionOffset({ object: player, ...offset })
      );
